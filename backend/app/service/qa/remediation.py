...
RULE_REMEDIATION_HINTS: dict[str, str] = {
    ...
    "rule_qa_semantic_audit": "{semantic_finding}",
}
...
def build_remediation_hints(
    ...
    *,
    semantic_rule_id: str = "rule_qa_semantic_audit",
) -> dict[str, str]:
    hints: dict[str, str] = {}
    for item in failed_rules:
        template = RULE_REMEDIATION_HINTS.get(item.rule_id)
        if template is None:
            hints[item.rule_id] = item.message
            continue
        if "{semantic_finding}" in template:
            hints[item.rule_id] = template.format(semantic_finding=item.message)
        else:
            hints[item.rule_id] = template
    if semantic_rule_id not in hints:
        for item in failed_rules:
            if item.rule_id == semantic_rule_id:
                hints[semantic_rule_id] = item.message
    return hints
