...
枚举为开放集，可在行业包内扩展，但 core 必须支持上述全部。
...
| `supervisor` | 单点修复无效，需要整体重新规划（如行业包选错） | Supervisor 重新决策路径 |

路由目标不可自由组合，必须与 `failed_rule_ids` / `semantic_findings` 的内容一致。

## 7. QA 规则 DSL（fast path）

第一版规则使用 YAML 配置，被 QA Reviewer 在 fast path 阶段全量加载。LLM 语义审查（slow path）的 prompt 由 `industry_packs/<pack>/qa_semantic_prompt.txt` 提供。
...
行业包可通过 `industry_packs/<pack>/qa_rules.yaml` 注册领域规则，被自动合并到 core 规则集。Skill Curator 产出的 `qa_rule_candidate` 在审核通过后写入对应 pack 的 `qa_rules.yaml`。
...
4. `approved` 候选自动写入对应 `industry_packs/<pack>/skills/` 文件（P1 范围）
...
```text
industry_packs/<pack_name>/
├── competitors.yaml          # 默认竞品列表
├── report_template.yaml      # 报告章节模板
├── qa_rules.yaml             # 领域 QA 规则（fast path）
├── qa_semantic_prompt.txt    # 领域 QA LLM 审查 prompt（slow path）
├── extension_schema.py       # 领域扩展 Pydantic 模型
├── researcher_tools/         # 可选的领域专用 Researcher 工具
└── skills/                   # Skill Curator 沉淀的生效 skill
    ├── prompt_templates/
    ├── source_routing.yaml
...
- 扩展字段**不得污染核心 Schema**（core schema 文件不允许 import 任何 pack）；
- 报告渲染可读取扩展字段生成行业章节；
- 换行业时新增另一个 extension model，**不改 Agent 主流程**；
- 行业包的所有可注册扩展点见 `docs/2.5-agent-architecture.md` §9。
...
|---|---|---|---|
| Supervisor | 用户 query、industry_pack、target_roles、Researcher fragments、QA rejection/approval | `ConductResearch` / `Analyze` / `Write` / `Finalize` tool call；每次落 `SupervisorDecision` | `delegation_request` / `supervisor_decision` |
...
| Writer | `Conclusion[]` + Evidence 引用 + `report_template.yaml` | `Report`（content_json + content_markdown） | `report_draft` |
