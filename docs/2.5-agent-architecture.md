...
```mermaid
flowchart TB
    User([用户 query<br/>domain_hint + reference_urls + roles])
...
| 工具集 | `DiscoverCompetitors(search_queries, domain_context)` / `ConductResearch(research_topic)` / `Analyze(focus_dimensions)` / `Write(template_id)` / `Finalize()` |
| 输入 | 用户 query、domain_hint、reference_urls、target_roles、已有 Researcher fragments、QA rejection |
| 输出 | tool_calls（含动态 K 个 Researcher 委派）或 Finalize 指令 |
| 决策权限 | 何时触发竞品发现（competitors 为空时）、何时 fan-out、fan-out 度 K、各 Researcher 的 topic、何时切换到 Analyst/Writer、何时结束 run |
...
sequenceDiagram
    ...
    U->>S: query + domain_hint + reference_urls + roles
    activate S
