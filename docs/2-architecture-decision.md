...
> - **Extensible**：Agent 角色、工具集、技能库、Schema 字段、QA 规则全部开放扩展。禁止把"当前实现的角色清单 / 工具清单 / 字段清单"写成代码闭集。当前实现中，`source_type` / `focus_dimension` / `section_id` 均改为通用字符串校验，不再是代码闭集。
...
- **通用核心 (core)**：跨场景稳定不变的机制与协议。修改 core 等于跨所有领域的兼容性变更。
- **技能库 (skill library)**：可热插拔的领域知识与提示模板，落在 `backend/skills/<applies_to>/<skill_id>/SKILL.md`，由 `SkillStore` 在 lifespan 启动时全量扫描。新增、替换或下线技能不应触及 core 代码。

> 早期版本曾以"行业包 (industry pack)"为扩展容器（fixed bundle，一个 run 必须先绑定到一个 pack）。该抽象在跑通 Researcher ReAct + Skill Curator 闭环后被下线，原因：① 答疑要求"通用竞品分析，不区分行业"（`docs/0-qna-signals.md`），fixed bundle 反向约束输入；② 实际进化产物（QA 规则 / prompt / source 偏好）粒度比"行业"细得多，按 `applies_to + tags` 切片更贴合 Agent 用法。
...
| 数据 | Evidence / Conclusion 绑定逻辑、脱敏流水线、Trace 与决策数据模型 | 数据源偏好（`applies_to=source_routing`）、QA 规则集（`applies_to=qa_rule`） |

具体的 core 类型清单与技能文件结构见 `backend/core/`、`backend/skills/<applies_to>/<id>/SKILL.md` 实现及 `docs/2.5-agent-architecture.md`、`docs/3-schema-and-protocol.md`、`docs/impl/13-skill-library.md`。

### 5.2 扩展契约

- 每个技能以独立目录承载：`backend/skills/<applies_to>/<skill_id>/SKILL.md`，YAML frontmatter（`id` / `applies_to` / `tags` / `version` / 可选 `severity` / `role` 等）+ Markdown body（规则 DSL 或 prompt 主体）；
- `applies_to ∈ {qa_rule, prompt_template, source_routing}` 由 core 定义为开放枚举，新增类型需 core 同步；
- core 通过 `SkillStore.list_by_applies_to(...)` / `load_skill` / `read_skill_file` 三个入口加载技能，**禁止 core 直接 import 技能文件内容**；
...
skill_candidates ──[approved by reviewer]──> backend/skills/<applies_to>/<id>/SKILL.md   （审核通过后落盘进生效池）
```
...
- 竞品知识 Schema 与技能库扩展机制（`SkillStore` + `applies_to/tags` + `domain_hint` 软匹配）；
