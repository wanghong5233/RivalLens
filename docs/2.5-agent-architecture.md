...
当前 ORCH-006 的落地边界：`qa_rule` 候选在 approve 后写入 `skills/qa_rules_promoted.yaml` 并由 pack loader 读取，QA fast path 以 observed-only 方式透传 `promoted_qa_rule_ids` 到 trace（不执行 YAML DSL reject）；规则 DSL 解释器留 ORCH-006b。
